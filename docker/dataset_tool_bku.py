...
def create_celebahq(tfrecord_dir, celeba_dir, delta_dir, num_threads=32, num_tasks=100):
    ...
    #             tfr.add_image(img)
    def do_the_work(img_num, img):
        print('Create image number {}'.format(img_num))
        # img = process_func(img_num)
        np.save(os.path.join(delta_dir, '%05d' % img_num), [img])
    
    with ThreadPool(num_threads) as pool:
        for idx, img in pool.process_items_concurrently(indices.tolist(), process_func=process_func, max_items_in_flight=num_tasks):
            do_the_work(idx, img)

    # with TFRecordExporter(tfrecord_dir, indices.size) as tfr:
        # order = tfr.choose_shuffled_order()
    # with ThreadPool(num_threads) as pool:
    #     for img in pool.process_items_concurrently(indices.tolist(), process_func=process_func, max_items_in_flight=num_tasks):
    #         tfr.add_image(img)
