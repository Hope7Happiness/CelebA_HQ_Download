    ...
    def process_items_concurrently(self, item_iterator, process_func=lambda x: x, pre_func=lambda x: x, post_func=lambda x: x, max_items_in_flight=None):
        ...
        def retire_result():
            ...
            while retire_idx[0] < len(results) and results[retire_idx[0]] is not None:
                ...
                # yield (retire_idx[0], post_func(results[retire_idx[0]]))
...
def create_celebahq(tfrecord_dir, celeba_dir, delta_dir, num_threads=32, num_tasks=100):
    ...
    # print a message that this pass
    print 'Congratulations! You have passed the first check!'
    #### END ADD FROM PREVIOUS CODE ####
    ...
    def process_func(idx):
        ...
        # print 'orig_idx: %d' % orig_idx
        orig_file = fields['orig_file'][idx]
        orig_path = os.path.join(celeba_dir, 'img_celeba', orig_file)
        img = PIL.Image.open(orig_path)
        # print img hash
        # md5 = hashlib.md5()
        # md5.update(np.array(img).tobytes())
        # print 'MD5 hash of original image: %s' % md5.hexdigest()
        ...
        md5.update(img.tobytes())
        A = md5.hexdigest()
        B = fields['proc_md5'][idx]
        assert A == B, 'MD5 mismatch: %s vs %s' % (A, B)
        print 'Passed proc md5 check: %d' % idx
        ...
        kdf = cryptography.hazmat.primitives.kdf.pbkdf2.PBKDF2HMAC(
            algorithm=algorithm,
            length=32,
            salt=orig_file, # here is different!
            iterations=100000,
            backend=backend)
        ...
        md5.update(img.tobytes())
        A = md5.hexdigest()
        B = fields['final_md5'][idx]
        assert A == B, 'MD5 mismatch: %s vs %s' % (A, B)
        print 'Passed final md5 check: %d !!' % idx
        # return img
        ...
        return orig_file, img64, img128, img256, img512, img, rawimg

    output_dir = '/data/main_sh_out/celeba-hq'
    ...
    with ThreadPool(num_threads) as pool:
        for orig_fn, aimg64, aimg128, aimg256, aimg512, aimg1024, npimg1024 in pool.process_items_concurrently(
                fields['idx'],
            ...
            # save a NPY
            np.save(img1024dir + os.sep + str(orig_fn).replace('jpg','npy'), npimg1024)
